...
type Key = keyof typeof TAILWIND_MAP;

export function mapStylesToClassNames(styles: Record<string, any>) {
  return Object.entries(styles)
    .map(([prop, value]) => {
      if (prop in TAILWIND_MAP) {
        if (typeof TAILWIND_MAP[prop as Key] === 'function') {
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
          return TAILWIND_MAP[prop as Key](value);
        } else if (value in TAILWIND_MAP[prop as Key]) {
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
          return TAILWIND_MAP[prop as Key][value];
        }
